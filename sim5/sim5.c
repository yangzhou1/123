...
WORD calc_branchAddr(WORD pcPlus4, InstructionFields *fields){
    //function  calculte the address we would jump to 
    //if we perform a conditional branch(BEQ,BNE,) 
...
//Take two pipeline register as input, return the correct ALU input1. Including the case when data forwarding happen.There are several cases.1:data forwading from ALU output2: dataforwarding from MEM phase
